...
ALGORITMO = "HS256"
MINUTOS_EXPIRA_ACCESO_TOKEN = 60
CLAVE_SECRETA = "jeiropro"
...
def crear_token_acceso(datos: dict, delta_expira: timedelta | None = None):
    ...
    return jwt.encode(codifica, CLAVE_SECRETA, algorithm=ALGORITMO)

def obtener_usuario_actual(db: Session = Depends(obtener_db), token: str = Depends(oauth2_scheme)):
    try:
...
def crear_usaurio(usuario: CrearUsuario, db: Session = Depends(obtener_db)):
    if obtener_usuario_correo_electronico(db, usuario.correoEletronico):
...
def inicio_sesion(datos_formulario: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(obtener_db)):
    ...
    token_acceso = crear_token_acceso({"sub": usuario.id}, timedelta(minutes=MINUTOS_EXPIRA_ACCESO_TOKEN))
    return {"token_acceso": token_acceso, "tipo_token": "bearer"}

@router.get("/jeironpro", response_model=RespuestaUsuario)
def jeironpro(usuario_actual: Usuario = Depends(obtener_db)):
